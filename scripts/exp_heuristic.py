...
def run_once(root: Path, seed: int, restarts: int, max_swaps: int, max_repairs: int, tabu: int, neighborhoods: str, weights_path: str | None) -> dict:
    cmd = [
        "python3",
        str(root / "scripts" / "run_heuristic.py"),
        "--seed", str(seed),
        "--restarts", str(restarts),
        "--max_swaps", str(max_swaps),
        "--max_repairs", str(max_repairs),
        "--tabu", str(tabu),
    ...
    if stamp:
        ...
        try:
            subprocess.check_call(["python3", str(root / "scripts" / "presubmit_check.py"), str(sched)], cwd=str(root))
...
def main() -> int:
    ...
    ap.add_argument("--neighborhoods", type=str, default="grade_day,grade_period,stuck_grade,blank_rr,kempe_period_swap")
    ...
    for (seed, restarts, max_swaps, max_repairs, tabu) in itertools.product(
        ...
        res = run_once(root, seed, restarts, max_swaps, max_repairs, tabu, args.neighborhoods, args.weights)
        results.append(res)
    ...
    with csv_path.open("w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow([
            "seed","restarts","max_swaps","max_repairs","tabu","neighborhoods","stamp",
            "blanks","conflicts","window_violations","fallback","adjacent","same_slot","penalty","presubmit_ok"
        ])
        for r in topN:
            m = r.get("metrics") or {}
            w.writerow([
                r.get("seed"), r.get("restarts"), r.get("max_swaps"), r.get("max_repairs"), r.get("tabu"), r.get("neighborhoods"), r.get("stamp"),
                m.get("blanks", 0), (m.get("teacher_conflicts", 0) + m.get("class_conflicts", 0)), m.get("window_violations", 0), m.get("fallback_supervised", 0), m.get("adjacent_repeats_extra", 0), m.get("same_slot_repeats", 0), m.get("penalty_sum", 0), r.get("presubmit_ok")
            ])
    ...
    for r in topN:
        print(f" - stamp={r.get('stamp')} seed={r.get('seed')} key={rank_key(r)} presubmit_ok={r.get('presubmit_ok')}")
